"""
Turkcell SQL AI  —  app.py  v6  (Clean Enterprise Layout)
"""
...
st.set_page_config(
    ...
    page_icon="🟡",
    layout="wide",                     # wide → kendi max-width'imizi biz yönetiyoruz
...
/* Kill every Streamlit background */
.stApp,
.stApp > div,
.block-container,
...
/* ═══ MAIN CONTAINER ════════════════════════════════════════════════════════
   max-width 860px, yatayda ortalı, header altından başlıyor
══════════════════════════════════════════════════════════════════════════════ */
.block-container {
...
.tc-hdr-left  { display: flex; align-items: center; gap: 11px; }
.tc-hdr-logo  {
  ...
  box-shadow: 0 1px 6px rgba(255,209,0,.35);
}
.tc-hdr-title {
  font-family: var(--sans);
...
/* ═══ MAIN PANEL ════════════════════════════════════════════════════════════
   Tek büyük beyaz panel — tüm form alanları burada yaşar
══════════════════════════════════════════════════════════════════════════════ */
.main-panel {
  background: #ffffff;
...
.stTextArea textarea:focus {
  ...
  box-shadow: 0 0 0 3px rgba(0,71,186,.09) !important;
...
.stButton > button {
  ...
  border-radius: var(--r-sm) !important;
  padding: .62rem 2.2rem !important;
  cursor: pointer !important;
  transition: background .15s, box-shadow .15s, transform .1s !important;
  box-shadow: 0 2px 10px rgba(255,209,0,.28) !important;
  width: auto !important;            /* auto width — panelde ortalı duracak */
...
.sql-card pre {
  ...
  font-size: .79rem !important; line-height: 1.78 !important;
  color: var(--text) !important;
...
.sql-badge {
  ...
  padding: 3px 10px;
  font-family: var(--sans); font-size: .61rem; font-weight: 600;
  color: var(--blue-dk);
}
.sql-badge .dot {
...
.alert-key {
  ...
  font-family:var(--sans); font-size:.82rem; display:flex;
  align-items:flex-start; gap:12px; margin-top:1.2rem; line-height:1.7;
}
.alert-key strong { color:#3B0764; }
.alert-key code {
  background:#ede9fe; padding:1px 5px; border-radius:4px;
  font-family:var(--mono); font-size:.74rem; color:#7C3AED;
}
.alert-key pre {
  background:#f5f3ff; border:1px solid #ddd6fe; border-radius:5px;
  padding:6px 11px; font-family:var(--mono); font-size:.74rem;
  color:#5B21B6; margin:.35rem 0 .15rem;
}
...
[data-testid="stSidebar"] {
  ...
  box-shadow:3px 0 20px rgba(0,20,80,.18);
}
[data-testid="stSidebar"] > div:first-child { padding:0 1.1rem 2rem !important; }

.sb-top {
  background:rgba(0,0,0,.20); margin:0 -1.1rem 1.4rem;
  padding:.85rem 1.1rem; border-bottom:1px solid rgba(255,255,255,.07);
  display:flex; align-items:center; gap:8px;
}
.sb-top-title {
  font-family:var(--sans); font-size:.68rem; font-weight:700;
  color:rgba(255,255,255,.88); letter-spacing:2px; text-transform:uppercase;
}
.sb-lbl {
  font-family:var(--sans); font-size:.56rem; font-weight:700;
  color:rgba(255,255,255,.40); letter-spacing:2.4px; text-transform:uppercase;
...
[data-testid="stFileUploaderDropzoneInstructions"] span {
  color:rgba(255,255,255,.35) !important; font-family:var(--sans) !important; font-size:.67rem !important;
}
[data-testid="stFileUploaderDropzone"] small {
  color:rgba(255,255,255,.22) !important; font-family:var(--sans) !important; font-size:.57rem !important;
}
.sb-schema-card {
  background:rgba(0,0,0,.22); border:1px solid rgba(255,255,255,.08);
  border-left:3px solid var(--yellow); border-radius:var(--r-sm);
  padding:.75rem .9rem; margin-top:.6rem; max-height:240px; overflow-y:auto;
}
.sb-schema-card pre {
...
.sb-status {
  ...
  margin-top:.6rem; font-family:var(--sans);
  font-size:.61rem; font-weight:600; color:#34D399;
}
.sb-status .dg { width:6px; height:6px; border-radius:50%; background:#34D399; flex-shrink:0; }
.sb-inactive { font-family:var(--sans); font-size:.67rem; color:rgba(255,255,255,.30); line-height:1.8; padding:.2rem 0; }
.sb-div { height:1px; background:rgba(255,255,255,.07); margin:.9rem 0; }
.sb-tip {
  font-family:var(--mono); font-size:.57rem;
  color:rgba(255,255,255,.32); line-height:1.85;
...
{sm.get(style,'')} Use CTEs for complexity. End with semicolon.
If not a SQL request: ERROR: Not a valid SQL request."""
    ...
    if uf:
        try:
            ...
            if chars > 80_000:
                ...
                ⚠ Dosya 80 KB limitini aşıyor.</div>""", unsafe_allow_html=True)
            else:
                schema_text = raw
                schema_meta = {"name":uf.name,"tables":tables,"chars":chars}
                prev = raw[:700]+("\n…" if len(raw)>700 else "")
                st.markdown(f"""
                <div class="sb-schema-card"><pre>{prev}</pre></div>
        ...
        st.markdown("""<p class="sb-inactive">Şema yüklenmedi.<br>
...
if not api_key:
    ...
    <code>.streamlit/secrets.toml</code> dosyasına ekleyin:</span>
    <pre>OPENAI_API_KEY = "sk-..."</pre>
    <span style="font-size:.70rem;opacity:.60;">
    Streamlit Cloud → App Settings › Secrets</span>
...
# ══════════════════════════════════════════════════════════════════════════════
# MAIN PANEL  —  tek büyük beyaz kart
# ══════════════════════════════════════════════════════════════════════════════
